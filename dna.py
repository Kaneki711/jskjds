...
#============================================================#
        if op.type == 26:
            ...
            if msg.contentType == 13:
            	if wait ["winfite"] == True:
            		if msg._from in mid or Owner:
            			_name = msg.contentMetadata["displayName"]
            			invite = msg.contentMetadata["mid"]
            			groups = ririn.getGroup(msg.to)
            			pending = group.invitee
            			target = []
            			for s in groups.member:
            				if _name in s.displayName:
            					ririn.sendMessage(msg.to,"-> ") + _name + " was invited "
            					break
            				elif invite in wait["blacklist"]:
            					ririn.sendMessage(msg.to,"Sorry You Can't Invite")
            					ririn.sendMessage("Call My Owner to Used")
            					break
            				else:
            			...
            			if targets == []:
            				...
            				for target in targets:
            					try:
            						ririn.findAndAddContactsByMid(taget)
            						ririn.inviteIntoGroup(msg.to,[target])
            						wait["winfite"] = False
            						break
            					except:
            						ririn.sendMessage(msg.to,"ERROR, Limite Invited")
            						wait["winfite"] = False
            						break
#============================================================#
        if op.type == 26:
            ...
            if msg.contentType == 13:
               ...
               if msg._from in admin:
                ...
                wait[ "winvite" ] = True
                ririn.sendMessage(msg.to, "sᴇɴᴅ ᴄᴏɴᴛᴀᴄᴛ ᴛᴏ ɪɴᴠɪᴛɪɴɢ" )
               ...
               if msg._from in admin:
                key = msg.text.replace("invite","")
                ririn.findAndAddContactsByMid(key)
                ririn.inviteIntoGroup(msg.to, [key])
                contact = ririn.getContact(key)                
               ...
               if msg._from in Owner:
                  ...
                  ririn.sendText(msg.to,"╔═════¤|{ Group List }|¤═════\n" + h + "╠═══════[  Total  ]════════\n║" + str(len(gid)) + "\n╚════════════════════")
...
#------------------------------------------------------------#
        ...
        if op.type == 15:
        	dan = ririn.getContact(op.param2)
        	tgb = ririn.getGroup(op.param1)
        	ririn.sendMessage(op.param1, "Selamat Jalan {}, Semogah tenang diluar sana {}\nGak usah balik lagi ya\nAwas kalau balik lagi 😘😘😘".format(str(dan.displayName),str(tgb.name)))
        	ririn.sendContact(op.param1, op.param2)
        	ririn.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
#----------------Fungsi Cek Sider-------------------#
        if op.type == 55:
