        ...
        if op.type == 11:
            if not op.param2 in Bots:
                if wait["CloseQR"] == True:
                  try:
                      kpist=[ririn,dna1,dna2,dna3,dna4]
            ...
            if op.param3 in mid:
              if op.param2 not in Bots or admin:
                try:
                  ...
                  json.dump(wait["blacklist"], f, sort_dna8ys=True, indent=4,ensure_ascii=False)
                except:
                        ...
                        for mention in mentionees:
                            if mid in mention["M"]:
                                if settings["detectMention"] == True:
                                    ...
                                    ririn.sendMessage(to, "Dasar Zomblo")
...
#-----------------------------------------------------------------#                    
                ...
                if settings["changePictureProfile"] == True:
                    ...
                    irene.sendMessage(to, "Berhasil mengubah foto profile")
                if msg.toType == 2:
                    if to in settings["changeGroupPicture"]:
                        path = irene.downloadObjectMsg(msg_id)
                        settings["changeGroupPicture"].remove(to)
                        irene.updateGroupPicture(to, path)
                        irene.sendMessage(to, "Berhasil mengubah foto group")           
#-------------------------------- Lurking ---------------------------------#
...
#-----------------------------------------------
              ...
              if msg._from in admin:
                ririn.sendText(msg.to, "Set Point")
                try:
                    ...
                    if msg.to in wait2['readPoint']:
                        if wait2["ROM"][msg.to].items() == []:
                            ...
                            for rom in wait2["ROM"][msg.to].items():
                                chiya += rom[1] + "\n"
                        ririn.sendText(msg.to, "||Di Read Oleh||%s\n||By : Dna Bot||\n\n>Pelaku CCTV<\n%s-=CCTV=-\n•Bintitan\n•Panuan\n•Kurapan\n•Kudisan\n\nAmiin Ya Allah\n[%s]" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                        ririn.sendText(msg.to, "Ketik Read dulu\nBaru Ketil View\nDASAR PEAK ♪")
...
#------------------------------------------------------------#
        if op.type == 17:
          if wait["wcOn"] == True:
            	...
            	ririn.sendText(op.param1,wait["joingc"] + "\n" + ririn.getContact(op.param2).displayName + "\nDi grup " + str(ginfo.name) + "\nCreator grup " + ginfo.creator.displayName + "\n\nBudayakan Cek Note\nGak boleh baperan ya  ")
            	ririn.sendMessage(c)
            	ririn.sendImageWithURL(op.param1,"http://dl.profile.line-cdn.net/" + contact.pictureStatus)
        ...
        if op.type == 15:
          if wait["leftOn"] == True:
            	...
            	c.contentMetadata={'mid':op.param2}
            	ririn.sendMessage(c)
            	ririn.sendText(op.param1,ririn.getContact(op.param2).displayName + "\n" + wait["leftgc"])
