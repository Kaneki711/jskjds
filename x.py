...
#-------------Invite User By Contact-------------#
            if msg.contentType == 13:
                if wait['invite'] == True:
                     ...
                     groups = dna2.getGroup(msg.to)                     
                     groups = dna3.getGroup(msg.to)                     
                     groups = dna4.getGroup(msg.to)                
                     groups = dna5.getGroup(msg.to)
                     groups = dna6.getGroup(msg.to)                     
                     groups = dna7.getGroup(msg.to)                     
                     groups = dna8.getGroup(msg.to)                     
                     groups = dna9.getGroup(msg.to)                                          
                     pending = groups.invitee
                     targets = []
                     for s in groups.members:
                         if _name in s.displayName:
                             random.choice(KAC).sendText(msg.to, _name + " sᴜᴅᴀʜ ᴅɪ ᴅᴀʟᴀᴍ ɢʀᴜᴘ")
                         else:
                     ...
                     if targets == []:
                         ...
                         for target in targets:
                             try:
                                 ...
                                 dna2.findAndAddContactsByMid(target)                                 
                                 dna3.findAndAddContactsByMid(target)                                 
                                 dna4.findAndAddContactsByMid(target)                                 
                                 dna5.findAndAddContactsByMid(target)
                                 dna6.findAndAddContactsByMid(target)                                 
                                 dna7.findAndAddContactsByMid(target)                                 
                                 dna8.findAndAddContactsByMid(target)                                 
                                 dna9.findAndAddContactsByMid(target)                     
                                 random.choice(KAC).inviteIntoGroup(msg.to,[target])
                                 random.choice(KAC).sendText(msg.to,"Invite " + _name)
                                 wait['invite'] = False
                                 break                              
                             except:             
                                      random.choice(KAC).sendText(msg.to,"ᴇʀʀᴏʀ")
                                      wait['invite'] = False
                                      break
#----------------Notif Leave Group----------------#
        if op.type == 15:
